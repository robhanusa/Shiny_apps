...
order_1 <- reactive(
            c('mat_1' = ifelse(input$include_order1,input$mat1_1,0),
            'mat_2' = ifelse(input$include_order1,input$mat2_1,0),
            'week' = 10)
            ) 
order_2 <- reactive(
            c('mat_1' = ifelse(input$include_order2,input$mat1_2,0),
             'mat_2' = ifelse(input$include_order2,input$mat2_2,0),
             'week' = 24)
              )
order_3 <- reactive(
              c('mat_1' = ifelse(input$include_order3,input$mat1_3,0),
...
include_prod2 <- reactive(input$include_prod2)
prod2_start <-reactive(input$prod2_start)
include_prod3 <- reactive(input$include_prod3)
prod3_start <- reactive(input$prod3_start)
...
stock2 <- reactive(calc_stock(input$mat2_start,cons2_tot,'mat_2'))
